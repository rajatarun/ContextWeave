...
**(c) A shared telemetry sink.** The spans that the gate emits land in one table, written by ToolWeave, TeamWeave, DeviceWeave, RoutineWeave and ContextWeave, and read by exactly one endpoint: TeamWeave `GET /observability/agent-metrics`. The data plane is already unified; the reading of it is not.
...
What the whole-portfolio read adds to that sentence: **the data plane is already unified** (one metrics table, five writers) even though the reading of it is not; **the identity layer exists** and is adopted by one API; **a flywheel is designed** (preference pairs → LoRA → adapters → aliases) and disconnected; and **a second architectural idea recurs unnamed** (offline policy compilation). The market position — governed execution for agents, open-source gate as the wedge, AWS reference architecture second — stands; E1–E2 are its prerequisites, and E3–E6 are what make "platform" a true description.
